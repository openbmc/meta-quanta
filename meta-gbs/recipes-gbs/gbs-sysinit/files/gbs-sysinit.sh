...
check_board_ver() {
  ...
  #
  # TODO: might want to add SKU_ID check if want to use the same code
  # for Cryptids.
  # BOARD_REV_ID[7:6] =
  # 0x00 - EVT
  # 0x01 - DVT
...
set_specific_gpio_def() {
  GPIO_NAMES_TO_NUMBER["PWRGD_OUT_CPU_BMC"]=20
  GPIO_NAMES_TO_NUMBER["RST_BMC_RSMRST_N"]=173
}
...
set_hdd_prsnt() {
  ...
  local sata_prsnt_n
  set_gpio_direction 'SATA0_PRSNT_N' in
  if ! sata_prsnt_n="$(get_gpio_value 'SATA0_PRSNT_N')"; then
    return 1
  fi
  # sata_prsnt_n is active low => value 1 means low
  if [[ ${sata_prsnt_n} == 1 ]]; then
...
parse_pe_fru() {
  ...
  for i in {1..2};
  do
     if [ $(get_gpio_value 'FM_SLOT'$(($i-1))'_PRSNT_N') -eq 1 ] ;then
         pe_fruid=$(($pe_fruid+1))
         continue
     fi

     pe_fru_bus="$(ls -al /sys/bus/i2c/drivers/pca954x/0-0071/ | grep channel | awk -F "/" '{print $(NF)}' | awk -F "-" '{print $2}' | sed -n "${i}p")"
...
check_power_status() {
    res0="$(busctl get-property xyz.openbmc_project.State.Chassis \
        /xyz/openbmc_project/state/chassis0 xyz.openbmc_project.State.Chassis \
        CurrentPowerState --no-pager | awk '{print $2}' | sed s/\"//g | cut -d . -f 6)"
    echo $res0
}
...
main() {
  ...
  check_board_sku
  if [[ "${BOARD_SKU}" == "GBS" ]]; then
    hsbp_prsnt="$(get_gpio_value 'HSBP_CABLE_PRSNT_BUF_N')"
    if [[ ${hsbp_prsnt} == 0 ]]; then
      get_hsp_board_rev_id
      hsbp_12v_prsnt="$(get_gpio_value 'HSBP_P12V_STBY_CABLE_PRSNT')"
      if [[ ${hsbp_12v_prsnt} != 1 ]]; then
    ...
    fan_prsnt="$(get_gpio_value 'FAN_BRD_CABLE_PRSNT_BUF')"
    if [[ ${fan_prsnt} == 1 ]]; then
      get_fan_board_rev_id
    else
