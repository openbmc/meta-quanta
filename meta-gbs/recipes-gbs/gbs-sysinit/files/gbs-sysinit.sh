...
PRESENT_OBJPATH=("/xyz/openbmc_project/inventory/system/chassis/gpios/pe_slot0_prsnt"
...
set_hdd_prsnt() {
  ...
  sata_prsnt_n="$(busctl get-property $SERVICE_NAME ${PRESENT_OBJPATH[5]} \
...
parse_pe_fru() {
  ...
  for i in {1..2};
  do
     pe_prsnt_n="$(busctl get-property $SERVICE_NAME ${PRESENT_OBJPATH[$(($i-1))]} \
                  $INTERFACE_NAME Present | awk '{print $2}')"
...
main() {
  ...
  hsbp_prsnt="$(busctl get-property $SERVICE_NAME ${PRESENT_OBJPATH[2]} \
               $INTERFACE_NAME Present | awk '{print $2}')"
  if [[ ${hsbp_prsnt} == "true" ]]; then
    get_hsp_board_rev_id
    hsbp_12v_prsnt="$(busctl get-property $SERVICE_NAME ${PRESENT_OBJPATH[4]} \
                     $INTERFACE_NAME Present | awk '{print $2}')"
  ...
  fan_prsnt="$(busctl get-property $SERVICE_NAME ${PRESENT_OBJPATH[3]} \
              $INTERFACE_NAME Present | awk '{print $2}')"
